...
def make_crop_data_batch(render_size, ob_in_cams, mesh, rgb, depth, K, crop_ratio, normal_map=None, mesh_diameter=None, glctx=None, mesh_tensors=None, dataset:TripletH5Dataset=None, cfg=None):
  ...
  bs = 512
  verts_n = int(getattr(mesh.vertices, "shape", [1])[0]) if hasattr(mesh, "vertices") else 1
  bytes_per_pose = max(1, verts_n * 3 * 4)  # float32 xyz per vertex
  target_pts_cam_bytes = 64 * 1024 * 1024
  bs_adaptive = max(1, int(target_pts_cam_bytes // bytes_per_pose))
  score_render_bs_cap = 8
  if cfg is not None:
    if 'score_render_bs_cap' in cfg and cfg['score_render_bs_cap'] is not None:
      score_render_bs_cap = int(cfg['score_render_bs_cap'])
    elif 'render_bs_cap' in cfg and cfg['render_bs_cap'] is not None:
      score_render_bs_cap = int(cfg['render_bs_cap'])
  bs = int(max(1, min(bs, bs_adaptive, B, score_render_bs_cap)))
  logging.info(
    f"score.make_crop_data_batch bs={bs}, bs_adaptive={bs_adaptive}, "
    f"score_render_bs_cap={score_render_bs_cap}, B={B}, verts_n={verts_n}"
  )
  rgb_rs = []
  depth_rs = []
  xyz_map_rs = []
...
class ScorePredictor:
  ...
  def predict(self, rgb, depth, K, ob_in_cams, normal_map=None, get_vis=False, mesh=None, mesh_tensors=None, glctx=None, mesh_diameter=None):
    ...
    def find_best_among_pairs(pose_data:BatchPoseData):
      ...
      ids = []
      scores = []
      score_forward_bs_cap = 16
      if 'score_forward_bs_cap' in self.cfg and self.cfg['score_forward_bs_cap'] is not None:
        score_forward_bs_cap = int(self.cfg['score_forward_bs_cap'])
      bs = int(max(1, min(pose_data.rgbAs.shape[0], score_forward_bs_cap)))
      logging.info(f'score forward bs={bs}, score_forward_bs_cap={score_forward_bs_cap}')

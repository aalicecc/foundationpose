...
def make_crop_data_batch(render_size, ob_in_cams, mesh, rgb, depth, K, crop_ratio, xyz_map, normal_map=None, mesh_diameter=None, cfg=None, glctx=None, mesh_tensors=None, dataset:PoseRefinePairH5Dataset=None):
  ...
  bytes_per_pose = max(1, verts_n * 3 * 4)  # float32 xyz map for one pose
  target_pts_cam_bytes = 64 * 1024 * 1024
  bs_adaptive = max(1, int(target_pts_cam_bytes // bytes_per_pose))
  render_bs_cap = 64
  if cfg is not None and 'render_bs_cap' in cfg and cfg['render_bs_cap'] is not None:
    render_bs_cap = int(cfg['render_bs_cap'])
  bs = int(max(1, min(bs, bs_adaptive, B, render_bs_cap)))
  logging.info(f"make_crop_data_batch bs={bs}, bs_adaptive={bs_adaptive}, render_bs_cap={render_bs_cap}, B={B}, verts_n={verts_n}")
  rgb_rs = []
  depth_rs = []
  normal_rs = []
  xyz_map_rs = []

...
def torch_safe_load(weight):
    ...
    try:
        ...
        # PyTorch>=2.6 defaults torch.load(weights_only=True), which breaks some legacy checkpoints.
        if 'Weights only load failed' in str(e):
            LOGGER.warning('Falling back to torch.load(weights_only=False) for trusted legacy checkpoint.')

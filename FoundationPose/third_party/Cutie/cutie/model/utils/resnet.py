...
def resnet18(pretrained=True, extra_dim=0):
    ...
    if pretrained:
        source_state, _, _ = _with_cpu_default_tensor_type(
            lambda: model_zoo.load_url(model_urls['resnet18'])
        )
        load_weights_add_extra_dim(model, source_state, extra_dim)
    return model
...
def resnet50(pretrained=True, extra_dim=0):
    ...
    if pretrained:
        source_state, _, _ = _with_cpu_default_tensor_type(
            lambda: model_zoo.load_url(model_urls['resnet50'])
        )
        load_weights_add_extra_dim(model, source_state, extra_dim)
    return model

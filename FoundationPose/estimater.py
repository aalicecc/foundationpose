...
class FoundationPose:
  def __init__(
    ...
    rot_grid_min_views=20,
    rot_grid_inplane_step=90,
    score_max_candidates=64,
  ):
    self.gt_pose = None
    ...
    self.make_rotation_grid(min_n_views=rot_grid_min_views, inplane_step=rot_grid_inplane_step)
  ...
  def register(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5):
    ...
    if self.score_max_candidates > 0 and len(poses) > self.score_max_candidates:
      keep_ids = np.linspace(0, len(poses)-1, self.score_max_candidates, dtype=np.int64)
      keep_ids = torch.as_tensor(keep_ids, device=poses.device, dtype=torch.long)
      logging.info(f"score candidate cap: {len(poses)} -> {len(keep_ids)}")
      poses = poses[keep_ids]
  ...
  def track_one(self, rgb, depth, K, iteration, extra={}, fast_depth_filter=False):
    ...
    if fast_depth_filter:
      depth = erode_depth(depth, radius=1, device='cuda')
    else:
      depth = erode_depth(depth, radius=2, device='cuda')
      depth = bilateral_filter_depth(depth, radius=2, device='cuda')
    if self.debug >= 1:
